...
import { fetchStats, fetchMembers, fetchPending, updateStatus } from "./api.js";
...
function renderPagination(page, pages) {
  ...
  for (let i = 1; i <= pages; i += 1) {
    ...
    btn.textContent = i;
    btn.disabled = i === page;
    btn.addEventListener("click", () => loadMembers(i));
...
function renderPending(list) {
  ...
  list.forEach((item) => {
    ...
    acceptBtn.addEventListener("click", async () => {
      ...
      createToast("تم قبول الطلب", "success");
      loadPending();
    });
...
async function loadMembers(page = 1) {
  ...
  cachedMembers = response.items || [];
  updatePlanOptions(cachedMembers);
  applySearch();
  renderPagination(response.page || 1, response.pages || 1);
}
